...
const Navbar = () => {
    ...
    useEffect(() => {
        const handleClickOutside = event => {
            if (refAvatar && refAvatar.current && !refAvatar.current.contains(event.target)) {
                setAvatarMenu(false)
            }
            if (refHamburger && refHamburger.current && !refHamburger.current.contains(event.target)) {
                ...
                <div className='flex flex-row gap-5'>
                    {!user ?
                        ...
                        <div className="flex items-center md:order-2">
                            ...
                            <div ref={refAvatar} className={`${avatarMenu ? '' : 'hidden'} z-50 flex flex-col absolute justify-between w-full top-11 left-0 my-4 text-center list-none bg-white divide-y divide-gray-100 rounded`} id="user-dropdown">
