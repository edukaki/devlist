    ...
    <div className="grid grid-cols-4 md:grid-cols-5 text-center py-7 border border-b-orange">
      {props.check === true ? (
        <div className="flex flex-row gap-5 col-span-2 md:col-span-1 text-left">
          <input type="checkbox" className="w-[2rem]" onChange={handleChange} />
      ...
      <span className="hidden md:block">
        {props.client ? props.client : 'Personal'}
      </span>
      <span className="hidden md:block">{props.start}</span>
      <span
        className={`${
          new Date(props.deadline).setHours(0, 0, 0, 0) <
            today.setHours(0, 0, 0, 0) && props.status !== 'Closed'
