const Heading = (props) => {
    ...
    const expression = () => (
        type === "headingOne" ?
            <h1 className={`${line()} bg-gradient font-courier text-3xl font-bold text-darkGray pl-2 bg-lightGray`}>{props.headingOneContent}</h1> :
            type === "headingTwo" ?
                <h2 className={`${line()} inline font-courier text-left text-xl font-semibold text-darkGray`}>{props.headingTwoContent}</h2> :
                ''
    )
