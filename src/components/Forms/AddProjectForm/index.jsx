...
const AddProjectForm = () => {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      client: Yup.string().max(20, 'Must be 20 characters or less'),
      status: Yup.string().required('Required'),
      start: Yup.date().required('Required'),
      deadline: Yup.date()
        .min(
          Yup.ref('start'),
          'Deadline can not be earlier than the start date',
        )
        .required('Required'),
    }),
  ...
  return (
    <form
      ...
      className="project-form flex flex-col py-6 font-courier text-darkGray space-y-3 m-auto max-w-[600px]"
    >
